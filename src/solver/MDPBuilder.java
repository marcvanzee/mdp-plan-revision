...
public class MDPBuilder {
    ...
    */
	...
	private State[][] generateStates() {
    	...
    	State[][] states = new State[maxX][maxY];
    	...
    	{    		
    		for (int j=0; j<maxY; j++) 
    		{
    			states[i][j] = (test_p(pState) ? 
    					new State(rand_double(minReward, maxReward), "" + num, i, j):
    						null);
	...
	private void generateTransitions(State[][] states) {
		int numActions = Parameters.numActions, maxX = Parameters.maxX, maxY = Parameters.maxY;
		double pExecutable = Parameters.pExecutable, pDeterministic = Parameters.pDeterministic;
